...
final class BrowserScheduleTests: XCTestCase {
  ...
  func testParseValidTime() {
    XCTAssertEqual(parseTime("09:00"), 540)
    XCTAssertEqual(parseTime("18:30"), 1110)
    XCTAssertEqual(parseTime("00:00"), 0)
    XCTAssertEqual(parseTime("23:59"), 1439)
  }

  func testParseInvalidTime() {
    // Note: parseTime accepts single digit hours
    XCTAssertEqual(parseTime("9:00"), 540)  // Single digit hour is valid
  ...
  func testGetBrowserForURLWorkTime() {
    ...
    XCTAssertEqual(
      getBrowserForURL("https://example.com", config: config, currentDate: workTime), "Chrome"
    )
  }
  ...
  func testGetBrowserForURLPersonalTime() {
    ...
    XCTAssertEqual(
      getBrowserForURL("https://example.com", config: config, currentDate: personalTime),
      "Zen")
  }
  ...
  func testGetBrowserForURLPersonalOverride() {
    ...
    XCTAssertEqual(
      getBrowserForURL(
        "https://reddit.com/r/programming", config: config, currentDate: workTime), "Zen")
  }
  ...
  func testGetBrowserForURLWorkOverride() {
    ...
    XCTAssertEqual(
      getBrowserForURL(
        "https://mycompany.com/dashboard", config: config, currentDate: personalTime),
      "Chrome")
  }
  ...
  func testGetBrowserForURLInvalidURL() {
    ...
    XCTAssertEqual(
      getBrowserForURL("invalid-url", config: config, currentDate: workTime), "Chrome")
  }
  ...
  func testMinutePrecisionWorkTime() {
    let config = Config(
      workTime: Config.WorkTime(start: "9:30", end: "17:45"),
      workDays: Config.WorkDays(start: "Mon", end: "Fri")
    )
    ...
    // 9:15 - before work (should be false)
    components.hour = 9
    components.minute = 15
    let before = Calendar.current.date(from: components)!
    XCTAssertFalse(isWorkTime(config: config, currentDate: before))

    // 9:30 - exactly at start (should be true)
    components.minute = 30
    let atStart = Calendar.current.date(from: components)!
    XCTAssertTrue(isWorkTime(config: config, currentDate: atStart))

    // 17:44 - just before end (should be true)
    components.hour = 17
    components.minute = 44
    let justBefore = Calendar.current.date(from: components)!
    XCTAssertTrue(isWorkTime(config: config, currentDate: justBefore))

    // 17:45 - exactly at end (should be false)
    components.minute = 45
    let atEnd = Calendar.current.date(from: components)!
    XCTAssertFalse(isWorkTime(config: config, currentDate: atEnd))
  }
