...
public struct Config: Codable {
  ...
  public struct WorkTime: Codable {
    ...
    public var startMinutes: Int? { parseTime(start) }
    public var endMinutes: Int? { parseTime(end) }
    public var isNightShift: Bool {
      guard let start = startMinutes, let end = endMinutes else { return false }
      return start >= end
    }
...
public func isWorkTime(config: Config, currentDate: Date = Date()) -> Bool {
  ...
  guard validation.isValid,
    let startMinutes = config.workTime.startMinutes,
    let endMinutes = config.workTime.endMinutes,
  ...
  logger.debug(
    "\(shiftType) shift check: weekday=\(weekday), workDays=\(config.workDays.start)-\(config.workDays.end), currentMinutes=\(currentMinutes), workHours=\(config.workTime.start)-\(config.workTime.end), isWorkDay=\(isWorkDay), isWorkHour=\(isWorkHour)"
  )
